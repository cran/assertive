...
As well as **assertive** itself, there are lots of packages named **assertive.\***.
For example, **assertive.numbers**, **assertive.data**, and **assertive.types**.  
**assertive** doesn't actually contain any functions of its own.  All it does is 
re-export all functions from these **assertive.\*** packages.  Why is this a good 
thing you may ask?

For *interactive* use, typing less is a blessing.  So when you use R interactively,
so can just type `library(assertive)`, and have access to all the functionality 
without having to remember which package it comes from.

For *programmatic* use, particularly for developing other packages that depend upon 
*assertive*, you can use only the **assertive.\*** packages that you require, giving 
your code a smaller footprint.
...
5. Use **assertive** interactively and **assertive.\*** programmatically.
